...
class RequestDetailView(SupportDashboardMixin, View):
    ...
    def get(self, request, request_type, pk, *args, **kwargs):
        ...
        if request_type == 'complaint':
            request_obj = get_object_or_404(Complaint, pk=pk)
        elif request_type == 'service':
            request_obj = get_object_or_404(ServiceRequest, pk=pk)
        elif request_type == 'inquiry':
            request_obj = get_object_or_404(Inquiry, pk=pk)
        elif request_type == 'emergency':
            request_obj = get_object_or_404(EmergencyReport, pk=pk)
        else:
    ...
    def post(self, request, request_type, pk, *args, **kwargs):
        ...
        if 'update_status' in request.POST:
            ...
            if assignment_form.is_valid():
                request_obj.assigned_to = assignment_form.cleaned_data['assigned_to']
                request_obj.save()
